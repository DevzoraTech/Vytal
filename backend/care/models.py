...
class ModuleAccess(models.Model):
    class Module(models.TextChoices):
        ...
        CONSULTATION = "consultation", "Consultation & Review"
...
class LabResult(SoftDeleteModel):
    ...
    status = models.CharField(
        max_length=24, choices=Status.choices, default=Status.COMPLETE
    )
...
class LabOrder(SoftDeleteModel):
    ...
    admission = models.ForeignKey(
        Admission, related_name="lab_orders", on_delete=models.PROTECT
    )
    triage_entry = models.ForeignKey(
        TriageEntry,
    ...
    clinical_question = models.CharField(max_length=255, blank=True)
    notes_to_lab = models.TextField(blank=True)
    policy_bypass = models.BooleanField(
        default=False, help_text="True if lab proceeded without formal order."
    )
    ...
    def __str__(self):
        return f"Lab order {self.pk} for admission {self.admission_id}"


class CarePlan(SoftDeleteModel):
    class Status(models.TextChoices):
        DRAFT = "draft", "Draft"
        FINALIZED = "finalized", "Finalized"

    admission = models.ForeignKey(
        Admission, related_name="care_plans", on_delete=models.PROTECT
    )
    ...
    plan_items = models.JSONField(default=dict, blank=True)
    ...
    def __str__(self):
        return f"Care plan v{self.version} for admission {self.admission_id}"


class ConsultationTask(SoftDeleteModel):
    class Status(models.TextChoices):
        OPEN = "open", "Open"
    ...
    class TaskType(models.TextChoices):
        NEW_PLAN = "new_plan", "New Care Plan"
        RESULTS_READY = "results_ready", "Results Ready"
        ACK_REQUIRED = "ack_required", "Acknowledgement Required"

    admission = models.ForeignKey(
        Admission, related_name="consultation_tasks", on_delete=models.PROTECT
    )
    ...
    target_role = models.CharField(max_length=64, blank=True)
    status = models.CharField(
    ...
    acknowledged_by = models.ForeignKey(
        User,
        related_name="acknowledged_tasks",
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
    )
    created_by = models.ForeignKey(
        User,
        related_name="created_consultation_tasks",
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
    )
    ...
    def __str__(self):
        return f"Task {self.task_type} for admission {self.admission_id}"


class ConsultationEvent(SoftDeleteModel):
    admission = models.ForeignKey(
        Admission, related_name="consultation_events", on_delete=models.PROTECT
    )
    event_type = models.CharField(max_length=64)
    payload = models.JSONField(default=dict, blank=True)
    occurred_at = models.DateTimeField()
    actor_name = models.CharField(max_length=150, blank=True)
    actor_role = models.CharField(max_length=150, blank=True)
    created_by = models.ForeignKey(
        User,
        related_name="created_consultation_events",
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
    )
    ...
    def __str__(self):
        return f"{self.event_type} for admission {self.admission_id}"
